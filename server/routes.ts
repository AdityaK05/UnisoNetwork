  ...
  app.post('/api/email/send-otp-signup', rateLimiters.otpSend, async (req: Request, res: Response) => {
    try {
      ...
      const sendViaSendGrid = async () => {
        ...
        // Use global fetch (Node 18+) with AbortController timeout
        ...
        try {
          ...
          console.log(`✅ Signup verification email sent via SendGrid to ${email}`);
          return true;
        } catch (err) {
          clearTimeout(id);
          throw err;
      ...
      const sendViaSmtp = async () => {
        ...
        console.log(`✅ Signup verification email sent via SMTP to ${email}`);
        return true;
      };

      // Try SendGrid first (HTTP), then SMTP, finally fallback to console logging
      try {
        ...
        // If SendGrid failed, try SMTP
        console.error('⚠️ Primary email provider failed:', emailError);
        try {
          await sendViaSmtp();
        } catch (smtpErr) {
